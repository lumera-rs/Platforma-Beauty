...
**How to apply:** Permit a production-marked account only when independent deployment and environment signals confirm a published production runtime. Default ambiguous or legacy accounts to production-safe blocking outside production. Payment documents and QR payloads must keep the snapshot captured when the obligation was issued. Paid purchase flows must validate server-owned payment instructions before opening their write transaction or emitting notifications, and translate configuration failures into a stable public unavailable response rather than exposing internal configuration codes.