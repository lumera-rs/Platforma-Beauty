...
**How to apply:** Layer explicit deployment indicators and `NODE_ENV` checks with strict disposable resource naming, manifest ownership, target identity matching, and loopback-only test services. Refuse ambiguous targets and never weaken the disposable-name boundary.