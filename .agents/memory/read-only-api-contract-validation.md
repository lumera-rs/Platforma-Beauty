...
**How to apply:** Keep the generator and its normalizer able to use a temporary output root. The validator should stage required generator inputs there, regenerate, normalize, compare every generated file to the tracked output, and clean up in a `finally` block. Error text must name the OpenAPI contract and each drifted generated artifact. Targeted browser regressions must keep this shared preflight; never add a special Playwright config that bypasses contract drift just to make one test run.