...
**How to apply:** Serialize obligation issuance on the subscription row and back it with partial unique indexes across pending renewal/upgrade kinds and each non-cancelled renewal service-period start. Snapshot the purchased plan, cycle, amount, and dates on the obligation. Quote deferred changes using the plan effective at the next boundary. If a future period is already paid, defer any later downgrade or cycle change until that paid period ends; activate the obligation snapshots at their boundary, never at settlement time.