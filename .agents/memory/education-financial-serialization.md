...
**How to apply:** Treat each service window as one payable entitlement and return the same obligation when instructions are requested again for that window. Keep its purchased plan, cycle, amount, and dates immutable. Worker delay must not shorten an unpaid period. If current or future coverage is paid, defer later downgrade, cycle, or custom-contract terms until it ends (or reject the change); activate prepaid terms only at their boundary.