...
export function prepareCiReviewedHistory(root: string): void {
  ...
  const probe = spawnSync("git", ["-C", root, "cat-file", "-e", `${PINNED_REVIEWED_EVIDENCE_COMMIT}^{commit}`]);
  if (probe.status !== 0) {
    // Fetch only the reviewed object through checkout's configured origin auth.
    // A denied/unavailable pin is a hard failure, not a working-tree fallback.
    execFileSync("git", ["-C", root, "fetch", "--no-tags", "--depth=1", "origin", PINNED_REVIEWED_EVIDENCE_COMMIT], {
      stdio: "pipe",
    });
  }
