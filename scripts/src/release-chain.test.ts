...
test("branch CI isolates database checks and orders browser journeys after every prerequisite", async () => {
  ...
  assert.equal(
    ...
    "export CI=true && pnpm run build && pnpm run test:internal-request-control-outputs && pnpm run test:beauty-marketplace-typecheck && pnpm run test:frontend-generated-typecheck && pnpm run test:api-server-typecheck && pnpm run test:browser-fixtures && pnpm run test:bundle-budget && pnpm run test:frontend-standards && pnpm run test:frontend-interactions",
