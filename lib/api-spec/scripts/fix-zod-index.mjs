...
import { apiOutputInventory } from "../api-output-inventory.mjs";
...
const outputRoot = process.env.API_CODEGEN_OUTPUT_ROOT
  ...
  : projectRoot;
const zodGeneratedDirectory = path.resolve(
  outputRoot,
  apiOutputInventory.generators.zod.source,
);
const clientGeneratedDirectory = path.resolve(
  outputRoot,
  apiOutputInventory.generators["api-client-react"].source,
);
const indexPath = path.resolve(zodGeneratedDirectory, "../index.ts");
const generatedApiPath = path.resolve(zodGeneratedDirectory, "api.ts");
...
const openApiPath = path.resolve(
  projectRoot,
  apiOutputInventory.publicDocumentation[0],
);
...
// UUID format is another Zod 4 top-level helper emitted by newer Orval.
generatedApi = generatedApi.replace(/\bzod\.uuid\(\)/g, "zod.string().uuid()");
