...
// Dependency manifests are untrusted release-check inputs. Keep these limits
// explicit so accepted work is bounded before recursive syntax validation.
export const MAX_DEPENDENCY_PACKAGE_BYTES = 1024 * 1024;
export const MAX_DEPENDENCY_PACKAGE_NESTING_DEPTH = 256;

function exceedsNestingDepth(source) {
  let depth = 0;
  let inString = false;
  let escaped = false;
  ...
  for (const character of source) {
    ...
    if (character === '"') {
      ...
      depth += 1;
      if (depth > MAX_DEPENDENCY_PACKAGE_NESTING_DEPTH) return true;
    } else if (character === "}" || character === "]") {
      depth -= 1;
    }
  }
...
export function parseDependencyPackageJson({ contents, label }) {
  if (Buffer.byteLength(contents, "utf8") > MAX_DEPENDENCY_PACKAGE_BYTES) {
    throw new Error(
      `${label} is invalid: DEPENDENCY_PACKAGE_JSON_TOO_LARGE`,
    );
  }
  if (exceedsNestingDepth(contents)) {
    throw new Error(
      `${label} is invalid: DEPENDENCY_PACKAGE_JSON_TOO_DEEP`,
    );
  }
