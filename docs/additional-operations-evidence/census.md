...
Owner literal cardinalities: Business Growth 96; Media 0; Shipping 1; Marketplace 0; Referral 1; Web Push 1; Booking Command 0; Education Bundle 4. The original census resolved by the manifest's `sourceArtifact.readCommand` includes the baseline’s per-owner DDL cardinality, all nine crosswalk IDs intentionally outside this DML/function census, and source checksums for all eight owner modules. The current `census.json` contains the commit/path/hash pointer and expected counts, not those literal rows or owner details.
...
Exact duplicate raw SQL: none. The pinned original census flags repeated terminal function names separately (including `reject_bundle_payment_reference_change` across L091/L095/L102). Different checksums mean same-name/repeated-target evidence alone is **not** proof of equivalent bodies or data effects; it remains a semantic-review overlap, not a deduplication conclusion.
