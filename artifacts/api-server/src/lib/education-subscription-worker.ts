...
export async function runEducationSubscriptionLifecycle(options?: {
  afterCandidatesLoaded?: (subscriptionIds: string[]) => Promise<void> | void;
}) {
  const now = new Date();
  const rows = await db.select({ subscription: educationCenterSubscriptionsTable, center: educationCentersTable, plan: subscriptionPlansTable, owner: usersTable })
    ...
    .where(or(
      and(eq(educationCenterSubscriptionsTable.status, "trial"), isNotNull(educationCenterSubscriptionsTable.trialEndsAt), lte(educationCenterSubscriptionsTable.trialEndsAt, addEducationBelgradeCalendarDays(now, 7))),
      and(eq(educationCenterSubscriptionsTable.status, "active"), isNotNull(educationCenterSubscriptionsTable.currentPeriodEnd), lte(educationCenterSubscriptionsTable.currentPeriodEnd, addEducationBelgradeCalendarDays(now, 7))),
      and(eq(educationCenterSubscriptionsTable.status, "active"), eq(educationCenterSubscriptionsTable.contractKind, "custom"), isNotNull(educationCenterSubscriptionsTable.contractEndsAt), lte(educationCenterSubscriptionsTable.contractEndsAt, addEducationBelgradeCalendarDays(now, 14))),
  ...
  await options?.afterCandidatesLoaded?.(rows.map((row) => row.subscription.id));
  let reminders = 0;
  let transitioned = 0;
  for (const row of rows) {
    ...
    if (due && due > now) {
      const days = educationBelgradeCalendarDayDifference(
        educationBelgradeDateKey(now),
        educationBelgradeDateKey(due),
      );
      ...
      if (bucket && row.owner.email) {
        const delivery = await enqueueTransactionalEmail(db, {
          eventKey: `education-subscription-expiry:${row.subscription.id}:${educationBelgradeDateKey(due)}:${bucket}`,
          ...
          metadata: { centerId: row.center.id, subscriptionId: row.subscription.id, daysRemaining: bucket },
        });
    ...
    await db.transaction(async (tx) => {
      ...
      if (!locked) return;
      const lockedClock = await tx.execute<{ now: Date }>(sql`select clock_timestamp() as now`);
      const lockedNow = new Date(lockedClock.rows[0]!.now);
      ...
      if ((locked.status === "trial" || locked.status === "active") && lockedDue && lockedDue.getTime() <= lockedNow.getTime()) {
        if (!locked.autoRenew) {
          ...
          await tx.update(educationCenterSubscriptionsTable).set({ status: "past_due", graceEndsAt: addEducationBelgradeCalendarDays(lockedNow, 5), updatedAt: lockedNow })
        ...
        const pendingChangeDue = Boolean(
          ...
          && locked.pendingPlanEffectiveAt <= lockedNow,
        );
        ...
        const [prepaidRenewal] = await tx.select().from(educationPaymentObligationsTable).where(and(
          ...
          eq(educationPaymentObligationsTable.status, "paid"),
          lte(educationPaymentObligationsTable.servicePeriodStart, lockedNow),
          gt(educationPaymentObligationsTable.servicePeriodEnd, lockedNow),
        )).limit(1);
        if (prepaidRenewal?.servicePeriodStart && prepaidRenewal.servicePeriodEnd) {
          ...
          await tx.update(educationCenterSubscriptionsTable).set({
            ...
            currentPeriodEnd: prepaidRenewal.servicePeriodEnd, graceEndsAt: null, deactivatedAt: null, updatedAt: lockedNow,
          ...
          if (extras.length) await tx.update(coursesTable).set({ published: false, subscriptionSuspended: true, updatedAt: lockedNow }).where(inArray(coursesTable.id, extras));
          ...
          if (available) {
            ...
            if (drafts.length) await tx.update(coursesTable).set({ published: true, subscriptionSuspended: false, updatedAt: lockedNow }).where(inArray(coursesTable.id, drafts.map((course) => course.id)));
          }
          transitioned += 1;
          return;
        }
        const graceEndsAt = addEducationBelgradeCalendarDays(lockedNow, 5);
         ...
         if (pendingUpgrade) {
            await tx.update(educationPaymentObligationsTable).set({ status: "cancelled", cancelledAt: lockedNow })
        ...
        const amount = locked.contractKind === "custom" ? locked.dueAmount : educationCycleAmount(nextPlan.price, nextCycle);
         ...
         const periodStart = locked.currentPeriodEnd && locked.currentPeriodEnd > lockedNow ? locked.currentPeriodEnd : lockedNow;
        ...
        await tx.update(educationCenterSubscriptionsTable).set({
          ...
          pendingPlanId: null, pendingBillingCycle: null, pendingPlanEffectiveAt: null,
           ...
           graceEndsAt, updatedAt: lockedNow,
        }).where(eq(educationCenterSubscriptionsTable.id, locked.id));
        ...
        transitioned += 1;
       } else if (locked.status === "past_due") {
         await tx.select({ id: coursesTable.id }).from(coursesTable)
           .where(eq(coursesTable.centerId, locked.centerId))
           .orderBy(asc(coursesTable.id)).for("update");
         const [latest] = await tx.select().from(educationCenterSubscriptionsTable)
           .where(eq(educationCenterSubscriptionsTable.id, locked.id)).limit(1);
         const deactivationClock = await tx.execute<{ now: Date }>(sql`select clock_timestamp() as now`);
         const deactivationNow = new Date(deactivationClock.rows[0]!.now);
         if (latest?.status === "past_due" && latest.graceEndsAt && latest.graceEndsAt <= deactivationNow) {
           await tx.update(educationCenterSubscriptionsTable).set({ status: "suspended", deactivatedAt: deactivationNow, updatedAt: deactivationNow }).where(eq(educationCenterSubscriptionsTable.id, locked.id));
           await tx.update(coursesTable).set({ published: false, subscriptionSuspended: true, updatedAt: deactivationNow }).where(and(eq(coursesTable.centerId, locked.centerId), eq(coursesTable.published, true)));
           transitioned += 1;
         }
      }
    });
