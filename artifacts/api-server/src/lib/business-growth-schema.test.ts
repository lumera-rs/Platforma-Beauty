...
async function run() {
  const s = TEST_SCHEMA;
  try {
    assert.equal(BUSINESS_GROWTH_SCHEMA_VERSION, 123, "v123 is the current production schema rollout");
    ...
    // ── v82 customer password setup capability rollout ─────────────────────
    assert.ok(await columnExists("users", "password_set_at"), "v82 users.password_set_at added to legacy users");
    for (const table of [
      "customer_password_setup_tokens",
      "customer_password_setup_audits",
      "customer_password_setup_rate_limits",
    ]) assert.ok(await objectExists(`SELECT to_regclass($1) IS NOT NULL AS exists`, [`${s}.${table}`]), `v82 ${table} exists`);
    const setupIndexes = (await q<{ indexname: string }>(
      `SELECT indexname FROM pg_indexes WHERE schemaname=$1 AND tablename='customer_password_setup_tokens'`,
      [s],
    )).rows.map((row) => row.indexname);
    ...
    await q(`INSERT INTO "${s}".customer_password_setup_tokens
      (user_id, issued_by_user_id, token_hash, expires_at) VALUES ($1, $2, repeat('a', 64), now() + interval '15 minutes')`,
      [setupTarget.id, setupAdmin.id]);
    await assert.rejects(
      q(`INSERT INTO "${s}".customer_password_setup_tokens
        (user_id, issued_by_user_id, token_hash, expires_at) VALUES ($1, $2, repeat('b', 64), now() + interval '15 minutes')`,
        [setupTarget.id, setupAdmin.id]),
      /duplicate key/i,
