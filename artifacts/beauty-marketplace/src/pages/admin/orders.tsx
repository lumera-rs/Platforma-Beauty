...
const formatOrderDate = (value: string | null, includeTime = false) => value
  ? includeTime
...
type PrintOrder = {
  id: string; createdAt: string | null; subtotal: number; shippingCost: number; total: number; deliveryMethod: string;
            ...
            <ol className="space-y-2">
              {order.fulfillmentHistory.map((event: any) => (
                <li key={event.id} className="border-l-2 border-primary/30 pl-3 text-xs">
                  ...
                  <br /><span className="text-muted-foreground">{event.createdAt ? new Date(event.createdAt).toLocaleString("sr-RS") : "—"}</span>
                </li>
              ))}
            </ol>
          </div>
...
function OrderPrintDocuments({ orders, mode }: { orders: PrintOrder[]; mode: "packing" | "invoice" }) {
  ...
  return <div className="print-root hidden print:block">
    ...
    {orders.map((order) => <section className="print-page" key={order.id}>
      <header className="print-header"><div><p className="print-label">LUMERA B2B</p><h1 className="print-title">{invoice ? "FAKTURA / RAČUN" : "OTPREMNICA"}</h1></div><div><p className="print-label">Broj porudžbine</p><b>#{order.id.slice(0, 8).toUpperCase()}</b><br /><span>{formatOrderDate(order.createdAt)}</span></div></header>
...
function AdminOrderDetail({ orderId }: { orderId: string }) {
  ...
  const couponDiscountRsd = (order as any).couponDiscountRsd;
  const saveOps = () => {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-3xl font-serif font-bold">Porudžbina #{order.id.slice(0, 8)}</h1><p className="text-muted-foreground">{order.salon.name} · {formatOrderDate(order.createdAt, true)}</p></div><div className="flex flex-wrap gap-2">{invoice && <Button asChild variant="outline" className="border-primary text-primary hover:bg-primary/5"><a href={`/api/admin/orders/${orderId}/invoice.pdf`} target="_blank" rel="noopener noreferrer" title={`Faktura ${invoice.number}`}><Download className="mr-2 h-4 w-4" />Server PDF Faktura</a></Button>}<Button variant="outline" onClick={() => printOrder("packing")}><Printer className="mr-2 h-4 w-4" />Štampaj otpremnicu</Button><Button variant="outline" onClick={() => printOrder("invoice")}><Printer className="mr-2 h-4 w-4" />Štampaj fakturu/račun</Button><Badge>{statusLabel[order.status]}</Badge><Badge variant="outline">{statusLabel[order.paymentStatus]}</Badge></div></div>
...
export default function AdminOrders() {
  ...
  return <AdminLayout>{retailMode ? <AdminRetailOrders /> : routeParams?.orderId ? <AdminOrderDetail orderId={routeParams.orderId} /> : <div className="space-y-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-3xl font-serif font-bold">B2B porudžbine</h1><p className="text-muted-foreground">Operativna obrada, dostava i plaćanja.</p></div><Button variant="outline" asChild><Link href="/admin/porudzbine?channel=retail">Retail porudžbine</Link></Button>{selected.length > 0 && <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={exportCsv}><Download className="mr-2 h-4 w-4" />CSV ({selected.length})</Button><Select value={bulkPrintMode} onValueChange={value => setBulkPrintMode(value as "packing" | "invoice")}><SelectTrigger className="w-48"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="packing">Otpremnice bez cena</SelectItem><SelectItem value="invoice">Fakture / računi</SelectItem></SelectContent></Select><Button variant="outline" onClick={printSelected}><Printer className="mr-2 h-4 w-4" />Štampaj ({selected.length})</Button><Select onValueChange={updateBulkStatus}><SelectTrigger className="w-44" disabled={bulk.isPending || actionGuard.isActive("bulk-status")}><SelectValue placeholder="Masovni status" /></SelectTrigger><SelectContent>{["confirmed", "shipped", "delivered", "cancelled"].map(s => <SelectItem key={s} value={s}>{statusLabel[s]}</SelectItem>)}</SelectContent></Select></div>}</div>
    ...
    {isLoading ? <Loader2 className="animate-spin" /> : <div className="space-y-3">{orders.map(order => {
      const invoice = (order as any).invoice as { number: string; issuedAt: string } | undefined;
      return <Card key={order.id}><CardContent className="flex flex-wrap items-center gap-3 p-4"><Checkbox checked={selected.includes(order.id)} onCheckedChange={() => toggle(order.id)} aria-label={`Izaberi ${order.id}`} /><div className="min-w-48 flex-1"><Link className="font-semibold hover:text-primary" href={`/admin/porudzbine/${order.id}`}>#{order.id.slice(0, 8)} · {order.salon.name}</Link><p className="text-sm text-muted-foreground">{order.delivery.recipientName} · {formatOrderDate(order.createdAt)}</p></div><Badge variant="outline">{order.deliveryMethod === "personal_belgrade" ? "Lična BG" : "Kurir"}</Badge><Badge>{statusLabel[order.status]}</Badge><Badge variant="secondary">{statusLabel[order.paymentStatus]}</Badge><b className="w-28 text-right">{money(order.total)}</b>{invoice && <Button asChild size="sm" variant="ghost" className="text-primary hover:text-primary hover:bg-primary/10" title="Preuzmi PDF Fakturu"><a href={`/api/admin/orders/${order.id}/invoice.pdf`} target="_blank" rel="noopener noreferrer"><Download className="h-4 w-4" /></a></Button>}<Button asChild size="sm" variant="outline"><Link href={`/admin/porudzbine/${order.id}`}>Detalji</Link></Button></CardContent></Card>
    })}</div>}
