...
import { CalendarDays, CheckCircle2, Clock3, Loader2, Plus, Repeat2, Trash2, UserRound, XCircle } from "lucide-react";
import { toast } from "sonner";
import { getGetCurrentUserQueryKey } from "@workspace/api-client-react";

type Appointment = { id: string; date: string; startTime: string; endTime: string; seriesId: string | null; status: "pending" | "confirmed" | "completed" | "cancelled" | "no-show"; notes: string | null; serviceName: string; customerName: string; customerPhone: string | null };
type Portal = {
  salon: { name: string };
...
export default function EmployeePortal() {
  ...
  const [seriesPreview, setSeriesPreview] = useState<{ slots: Array<{ date: string; startTime: string; available: boolean; reason: string | null }>; allAvailable: boolean } | null>(null);
  ...
  const book = async () => {
    try {
      if (seriesMode && !seriesPreview?.allAvailable) { toast.error("Prvo proverite da su svi termini iz serije slobodni."); return; }
      await api(seriesMode ? "/api/employee/appointment-series" : "/api/employee/appointments", { method: "POST", body: JSON.stringify({ serviceId: booking.serviceId, salonCustomerId: booking.salonCustomerId || undefined, guest: booking.salonCustomerId ? undefined : { firstName: booking.firstName, lastName: booking.lastName, phone: booking.phone, email: booking.email }, slots: booking.slots }) });
      toast.success(booking.slots.length > 1 ? "Termini su zakazani i potvrde poslate." : "Termin je zakazan i potvrda poslata.");
      setBookingOpen(false); setSeriesMode(false); setSeriesPreview(null); setBooking((current) => ({ ...current, salonCustomerId: "", firstName: "", lastName: "", phone: "", email: "", slots: [{ date: today(), startTime: "10:00" }] })); await load();
    } catch (error) { toast.error(error instanceof Error ? error.message : "Zakazivanje nije uspelo."); }
  };
  const previewEmployeeSeries = async () => {
    try {
      const preview = await api<{ slots: Array<{ date: string; startTime: string; available: boolean; reason: string | null }>; allAvailable: boolean }>("/api/employee/appointment-series/preview", { method: "POST", body: JSON.stringify({ serviceId: booking.serviceId, slots: booking.slots }) });
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "Dostupnost nije proverena."); }
  };
  ...
  return <BusinessLayout><div className="container mx-auto space-y-6 px-4 py-8">
    ...
    <Dialog open={bookingOpen} onOpenChange={setBookingOpen}><DialogContent className="max-h-[90vh] max-w-xl overflow-y-auto"><DialogHeader><DialogTitle>Zakaži termin za svog klijenta</DialogTitle></DialogHeader><div className="space-y-4"><div><Label>Usluga</Label><select className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm" value={booking.serviceId} onChange={(event) => setBooking({ ...booking, serviceId: event.target.value })}>{portal.services.map((service) => <option key={service.id} value={service.id}>{service.name} · {service.durationMinutes} min</option>)}</select></div><div><Label>Klijent kog ste ranije uslužili</Label><select className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm" value={booking.salonCustomerId} onChange={(event) => setBooking({ ...booking, salonCustomerId: event.target.value })}><option value="">Brzi unos novog klijenta</option>{portal.clients.map((client) => <option key={client.id} value={client.id}>{client.firstName} {client.lastName} · {client.phone ?? "bez telefona"}</option>)}</select></div>{!booking.salonCustomerId && <div className="grid gap-3 sm:grid-cols-2"><div><Label>Ime</Label><Input className="mt-1" value={booking.firstName} onChange={(event) => setBooking({ ...booking, firstName: event.target.value })} /></div><div><Label>Prezime</Label><Input className="mt-1" value={booking.lastName} onChange={(event) => setBooking({ ...booking, lastName: event.target.value })} /></div><div><Label>Telefon</Label><Input className="mt-1" value={booking.phone} onChange={(event) => setBooking({ ...booking, phone: event.target.value })} /></div><div><Label>Email (opciono)</Label><Input className="mt-1" type="email" value={booking.email} onChange={(event) => setBooking({ ...booking, email: event.target.value })} /></div></div>}<div className="rounded-lg border bg-muted/20 p-3"><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={seriesMode} onChange={(event) => { setSeriesMode(event.target.checked); setSeriesPreview(null); }} /><Repeat2 className="h-4 w-4 text-primary" /> Zakaži seriju termina</label>{seriesMode && <div className="mt-3 flex flex-wrap gap-2"><select className="h-9 rounded-md border bg-background px-2 text-sm" value={seriesRule} onChange={(event) => setSeriesRule(event.target.value)}><option value="daily">Svaki dan</option><option value="every-2-days">Svaka 2 dana</option><option value="every-3-days">Svaka 3 dana</option><option value="weekly">Nedeljno</option><option value="biweekly">Na 2 nedelje</option><option value="monthly">Mesečno</option></select><Input className="h-9 w-20" type="number" min="1" max="24" value={seriesCount} onChange={(event) => setSeriesCount(event.target.value)} /><Button type="button" size="sm" variant="outline" onClick={() => { setBooking({ ...booking, slots: recurringSlots(booking.slots[0]!, seriesRule, Number(seriesCount) || 1) }); setSeriesPreview(null); }}>Primeni</Button><Button type="button" size="sm" variant="outline" onClick={previewEmployeeSeries}>Proveri</Button></div>}</div><div className="space-y-2"><Label>Termini</Label>{booking.slots.map((slot, index) => { const state = seriesPreview?.slots.find((item) => item.date.slice(0, 10) === slot.date && item.startTime === slot.startTime); return <div className="flex items-center gap-2" key={index}><Input type="date" value={slot.date} onChange={(event) => { setBooking({ ...booking, slots: booking.slots.map((item, i) => i === index ? { ...item, date: event.target.value } : item) }); setSeriesPreview(null); }} /><Input type="time" value={slot.startTime} onChange={(event) => { setBooking({ ...booking, slots: booking.slots.map((item, i) => i === index ? { ...item, startTime: event.target.value } : item) }); setSeriesPreview(null); }} />{state && <span className={state.available ? "text-xs text-emerald-700" : "text-xs text-destructive"}>{state.available ? "Slobodno" : "Konflikt"}</span>}{booking.slots.length > 1 && <Button type="button" variant="outline" size="icon" aria-label="Ukloni termin" onClick={() => { setBooking({ ...booking, slots: booking.slots.filter((_, i) => i !== index) }); setSeriesPreview(null); }}><Trash2 className="h-4 w-4" /></Button>}</div>; })}<Button type="button" size="sm" variant="outline" onClick={() => { setBooking({ ...booking, slots: [...booking.slots, { date: booking.slots.at(-1)?.date ?? today(), startTime: "10:00" }] }); setSeriesPreview(null); }}><Plus className="mr-1 h-3.5 w-3.5" />Zakaži još jedan termin</Button></div><Button className="w-full" onClick={book}>Zakaži {seriesMode ? "seriju" : booking.slots.length > 1 ? "termine" : "termin"}</Button></div></DialogContent></Dialog>
