...
export default function MarketplaceGuides({ kind }: { kind: GuideKind }) {
  ...
  const endpoint = `/api/${kind === "inspiration" ? "inspiracija" : kind === "glossary" ? "recnik" : "brendovi"}`;
  const { data: filtered = [], isLoading: loading, isError } = useQuery<any[]>({
    queryKey: [endpoint, { query: debouncedQuery }],
    queryFn: async () => {
      const items = await queryClient.ensureQueryData<any[]>({
        queryKey: [endpoint],
        queryFn: async () => {
          const response = await fetch(endpoint);
          if (!response.ok) throw new Error(`Vodič nije dostupan (${response.status}).`);
          return response.json();
        },
      });
      return items.filter((item) => JSON.stringify(item).toLowerCase().includes(debouncedQuery.toLowerCase()));
    },
    retry: false,
  });
  ...
  return <Layout><main className="container mx-auto px-4 py-10 md:py-14">
    ...
    {isError ? <p role="alert" className="py-12 text-center">Vodič trenutno nije dostupan. Pokušajte ponovo.</p> : !loading && !filtered.length && <p className="py-12 text-center text-muted-foreground">Nema rezultata za izabranu pretragu.</p>}
  </main></Layout>;
}