...
export default function MarketplaceGuides({ kind }: { kind: GuideKind }) {
  ...
  return <Layout><main className="container mx-auto px-4 py-10 md:py-14">
    ...
    {loading ? <div className="py-16 flex justify-center"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : kind === "inspiration" ? <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">{filtered.map((item) => <article key={item.id} className="overflow-hidden rounded-2xl border bg-card"><img className="aspect-[4/3] w-full object-cover" src={item.imageUrl} alt={item.title} /><div className="p-4"><h2 className="font-serif text-xl font-bold">{item.title}</h2><div className="mt-2 flex gap-1 flex-wrap">{item.tags.map((tag: string) => <Badge key={tag} variant="secondary">{tag}</Badge>)}</div>{item.salon && <Link href={`/saloni/${item.salon.slug}`} className="mt-3 block text-sm text-primary font-medium">{item.salon.name} {item.serviceName ? `· ${item.serviceName}` : ""}</Link>}</div></article>)}</div> : <div className="grid md:grid-cols-2 gap-4">{filtered.map((item) => <article key={item.id} className="rounded-xl border bg-card p-5"><div className="flex justify-between gap-4"><h2 className="font-serif text-xl font-bold">{item.term ?? item.name}</h2>{item.category && <Badge variant="secondary">{item.category}</Badge>}</div><p className="mt-2 text-muted-foreground">{item.definition ?? item.description}</p>{item.salonCount !== undefined && <p className="mt-3 text-sm font-medium text-primary">{item.salonCount} salona koristi ovaj brend</p>}</article>)}</div>}
