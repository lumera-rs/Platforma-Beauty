...
import { Briefcase, MessageSquare, Bookmark, Bell, CornerDownRight, CalendarClock } from "lucide-react";
...
export default function BusinessBeautyJobsPage() {
  ...
  const isEmployee = currentUser?.user?.role === "SALON_EMPLOYEE";
  const canLoadOwnerTabs = Boolean(currentUser?.user) && !isEmployee;

  const requestedTab = searchParams.get("tab");
  const activeTab = isEmployee
    ? "my-jobs"
    : (requestedTab === "saved" || requestedTab === "inbox" || requestedTab === "rentals" || requestedTab === "notifications" ? requestedTab : "my-jobs");
  ...
  const shouldOpenNewListing = searchParams.get("new") === "1" && !isEmployee;

  const [editingJob, setEditingJob] = useState<BeautyJobListing | null>(null);
  const [isFormOpen, setIsFormOpen] = useState(false);

  const { data: myJobs, isLoading: isLoadingMyJobs } = useListMyBeautyJobs(undefined, { query: { enabled: canLoadOwnerTabs, queryKey: getListMyBeautyJobsQueryKey() } });
  const { data: savedJobs, isLoading: isLoadingSaved } = useListSavedBeautyJobs({ query: { enabled: canLoadOwnerTabs, queryKey: getListSavedBeautyJobsQueryKey() } });
  const { data: inbox, isLoading: isLoadingInbox } = useListBeautyJobInbox({ query: { enabled: canLoadOwnerTabs, queryKey: getListBeautyJobInboxQueryKey() } });
  const { data: notifications, isLoading: isLoadingNotifications } = useListBeautyJobNotifications({ query: { enabled: canLoadOwnerTabs, queryKey: getListBeautyJobNotificationsQueryKey() } });
  const { data: sentRentalRequests, isLoading: isLoadingSentRentalRequests } = useListMyBeautyJobRentalRequests({ query: { enabled: canLoadOwnerTabs, queryKey: getListMyBeautyJobRentalRequestsQueryKey() } });
  const { data: receivedRentalRequests, isLoading: isLoadingReceivedRentalRequests } = useListBeautyJobRentalRequestInbox({ query: { enabled: canLoadOwnerTabs, queryKey: getListBeautyJobRentalRequestInboxQueryKey() } });
  ...
  useEffect(() => {
    if (!requestedListingId || !myJobs?.items || isEmployee) return;
    const requestedListing = myJobs.items.find((job) => job.id === requestedListingId);
    if (!requestedListing) return;
    if (activeTab !== "my-jobs") handleTabChange("my-jobs");
    setEditingJob(requestedListing);
    setIsFormOpen(true);
  }, [myJobs?.items, requestedListingId, isEmployee]);

  useEffect(() => {
    if (!shouldOpenNewListing || isEmployee) return;
    if (activeTab !== "my-jobs") handleTabChange("my-jobs");
    setEditingJob(null);
    setIsFormOpen(true);
  }, [shouldOpenNewListing, isEmployee]);

  useEffect(() => {
    if (!requestedContactId || !inbox?.contacts?.some((contact) => contact.id === requestedContactId)) return;
    if (activeTab !== "inbox") handleTabChange("inbox");
    ...
    <BusinessLayout>
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          ...
          <TabsContent value="my-jobs" className="space-y-0">
            <BusinessJobsTab
              onEdit={(job) => { setEditingJob(job); setIsFormOpen(true); }}
              onNew={() => { setEditingJob(null); setIsFormOpen(true); }}
