...
{product.stock > 0 ? (
  ...
  <div className="mt-6 p-4 rounded-xl border bg-muted/20">
    ...
    {product.relatedProducts.length > 0 && <section data-testid="section-related-products"><h2 className="mb-4 text-xl font-bold">Slični proizvodi</h2><div className="grid grid-cols-2 gap-4 lg:grid-cols-4">{product.relatedProducts.map((related) => { const relatedPrice = related.discountPrice ?? related.price; return <Link key={related.id} href={`/vlasnik/shop/${supplierSlug}/proizvodi/${related.id}`} className="group overflow-hidden rounded-xl border bg-card" data-testid={`link-related-product-${related.id}`}><OptimizedImage src={related.imageUrl} alt={related.name} className="aspect-square w-full object-cover transition-transform group-hover:scale-105" width={320} height={320} preferredSize="thumbnail" /><div className="p-3"><p className="text-xs text-muted-foreground">{related.brand ?? "Proizvod"}</p><h3 className="mt-1 line-clamp-2 text-sm font-semibold">{related.name}</h3><p className="mt-2 font-semibold">{relatedPrice.toLocaleString("sr-RS")} RSD</p>{related.discountPrice != null && <p className="text-xs text-muted-foreground line-through">{related.price.toLocaleString("sr-RS")} RSD</p>}</div></Link>; })}</div></section>}
    <section><h2 className="text-xl font-bold mb-3">Recenzije {product.averageRating && <span className="text-sm font-normal">★ {product.averageRating} ({product.reviewCount})</span>}</h2><div className="space-y-3">{reviews.map((review) => <Card key={review.id}><CardContent className="p-4"><b>{review.salonName}</b><span className="ml-2 text-amber-500">{"★".repeat(review.rating)}</span><p className="text-sm mt-1">{review.comment}</p></CardContent></Card>)}</div>
      <Card className="mt-4"><CardContent className="p-4"><b>Ostavite ili izmenite svoju recenziju</b><div className="flex gap-2 mt-3 items-center"><Input className="w-24" type="number" min="1" max="5" value={rating} onChange={e => setRating(Number(e.target.value))}/><Input value={comment} onChange={e => setComment(e.target.value)} placeholder="Vaše iskustvo (opciono)"/><Button disabled={mutation.isPending} onClick={() => mutation.mutate({ productId: id, data: { rating, comment } }, { onSuccess: () => Promise.all([qc.invalidateQueries({ queryKey: getGetSupplierProductQueryKey(supplierSlug, id) }), qc.invalidateQueries({ queryKey: getListProductReviewsQueryKey(id) })]) })}><Star className="w-4 h-4 mr-1"/>Sačuvaj</Button></div></CardContent></Card></section>
