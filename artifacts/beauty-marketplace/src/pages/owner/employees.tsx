...
import { AlertTriangle, Check, Copy, KeyRound, Loader2, Plus, Trash2, Users } from "lucide-react";
...
export default function OwnerEmployees() {
  ...
  const [deactivation, setDeactivation] = useState<{ employee: Employee; futureAppointmentCount: number; hasLoginAccount: boolean } | null>(null);
  ...
  const uploadAvatar = async (file: File) => {
    ...
    try {
      ...
      setForm((current) => ({ ...current, avatarUrl: uploaded.imageUrl }));
      toast.success("Fotografija zaposlenog je optimizovana.");
    } catch (error) {
  ...
  const beginDeactivation = async (employee: Employee) => {
    try {
      ...
      setDeactivation({ employee, futureAppointmentCount: result.futureAppointmentCount, hasLoginAccount: result.hasLoginAccount });
    } catch (error) { toast.error(error instanceof Error ? error.message : "Radnja nije uspela."); }
  };
