...
import { AlertTriangle, Check, Copy, ImagePlus, KeyRound, Loader2, Plus, RotateCcw, Trash2, Users } from "lucide-react";
...
type Employee = { id: string; name: string; role: string; bio: string; avatarUrl: string; email: string | null; specialties: string[]; serviceIds: string[]; serviceNames: string[]; canOrderIndependently: boolean; active: boolean; account: { active: boolean; email: string; mustChangePassword: boolean } | null };
...
export default function OwnerEmployees() {
  ...
  const load = async () => {
    // includeInactive also surfaces employees whose assignment at this salon
    // was deactivated (Task #5), so they stay findable/reactivatable here
    // instead of disappearing entirely. Booking/scheduling pickers never
    // pass this flag, so they keep seeing active staff only.
    const [staff, catalog, requests, locationResponse] = await Promise.all([fetch("/api/salon/employees?includeInactive=true", { credentials: "include" }), fetch("/api/salon/services", { credentials: "include" }), fetch("/api/salon/leave-requests", { credentials: "include" }), fetch("/api/salon/managed-salons", { credentials: "include" })]);
    ...
    if (locationResponse.ok) {
      const locationBody = await locationResponse.json() as { activeSalonId: string | null; salons: ManagedLocation[] };
      setLocations(locationBody.salons);
      setActiveSalonId(locationBody.activeSalonId);
    }
    setLoading(false);
  };
  ...
  const reactivateEmployee = async (employee: Employee) => {
    if (!activeSalonId) { toast.error("Aktivna lokacija nije poznata."); return; }
    setReactivatingId(employee.id);
    try {
      // Reuses the same location-assignment endpoint the multi-location
      // checkbox already calls (Task #5's syncEmployeeAccountState then
      // recomputes employees.active/users.active/login from the assignment
      // rows -- the frontend never touches those fields directly).
      const response = await fetch(`/api/salon/employees/${employee.id}/locations/${activeSalonId}`, {
        method: "PUT", credentials: "include", headers: { "content-type": "application/json" },
        body: JSON.stringify({ active: true }),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error ?? "Zaposleni nije reaktiviran.");
      toast.success("Zaposleni je reaktiviran na ovoj lokaciji.");
      await load();
    } catch (error) { toast.error(error instanceof Error ? error.message : "Radnja nije uspela."); }
