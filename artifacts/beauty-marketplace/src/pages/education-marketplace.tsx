...
type Course = { id: string; title: string; description: string; format: "online" | "in-person" | "hybrid"; city: string | null; price: number; duration: number; certification: boolean; publisher: string; publisherVerified: boolean; sessions: { startsAt: string; availableSeats: number; location: string | null }[] };
...
export default function EducationMarketplace() {
  ...
  const buy = async (course: Course) => {
    ...
    setBuying(course.id);
    try {
      const response = await fetch(`/api/education/courses/${course.id}/enrollments`, { method: "POST", headers: { "content-type": "application/json", "idempotency-key": crypto.randomUUID() }, body: JSON.stringify({}) });
      ...
      setLocation(currentUser.user.role === "CUSTOMER" ? "/moj-nalog?tab=education" : "/biznis/edukacije");
